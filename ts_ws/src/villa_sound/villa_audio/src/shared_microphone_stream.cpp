...
SharedMicrophoneStream::SharedMicrophoneStream(uint timeout_s) {
  auto stamp_target = duration_cast<milliseconds>(system_clock::now().time_since_epoch()).count() + timeout_s * 1000;
  ...
  do {
    try {
      ...
      long stamp_now = duration_cast<milliseconds>(system_clock::now().time_since_epoch()).count();
      if (stamp_now > stamp_target) {
        throw_exception(std::runtime_error("Timed out waiting for shared memory"));
      }
    }
  } while(true);
